...
const {Purchase, Customer, Book, Sale} = require('../schemas')
...
const createWebPurchase = async (schema) => {
    const purchase = {
        type: 'Web',
        customer: schema.reader_card_num,
        books: JSON.parse(schema.books),
        spent_balance: 0,
        payment_method: schema.token.card.brand,
        subtotal: schema.subtotal,
        vat: VatRates.rates.PT.standard_rate,
        total: schema.total,
        shipping_method: schema.shipping_method,
        shipping_value: schema.shipping,
    }

    try {
        await customerExists(purchase)
        await calculateSubtotal(purchase.books)
...
const updateBooksStock = async (booksToUpdateStock, books) => {
    for (let i = 0; i < books.length; i++) {
        ...
        try {
            ...
            if(books[i].type == 'Used') {
                let previous = null
                let sales = await Sale.find({isbn: booksToUpdateStock[i].isbn, state: 'in_store'})
...
const getStatus = async (req, res, next) => {
    try {
        ...
        const cust = await Customer.findOne({reader_card_num: req.params.reader_card_num})

        let booksPurcNew = 0
        let booksPurcUsed = 0

        let purchases = await Purchase.find({customer: cust._id})
        ...
        let booksPurc = booksPurcUsed + booksPurcNew

        if (booksPurcUsed !== 0) {
            booksPurcUsed = (booksPurcUsed * 100) / booksPurc
        }

        if (booksPurcNew !== 0) {
            booksPurcNew = (booksPurcNew * 100) / booksPurc
        }

        return res.status(200).json(
            [
                {metric: 'Nº of books purchased', value: booksPurc},
                {metric: 'Percentage of used books purchased', value: booksPurcUsed.toFixed(1) + "%"},
                {metric: 'Percentage of new books purchased', value: booksPurcNew.toFixed(1) + "%"},
                {metric: 'Total spend in the store', value: total + "€"},
